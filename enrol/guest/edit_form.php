...
class enrol_guest_edit_form extends moodleform {
    ...
    function definition() {
        ...
        if (!$config) {
            $mform->hardFreeze('status');
            $mform->setConstant('status', $instance->status);
        }
        ...
        if (!$config) {
            ...
            $mform->setConstant('password', $instance->password);
        } else {
            $mform->disabledIf('password', 'status', 'noteq', ENROL_INSTANCE_ENABLED);
        }
