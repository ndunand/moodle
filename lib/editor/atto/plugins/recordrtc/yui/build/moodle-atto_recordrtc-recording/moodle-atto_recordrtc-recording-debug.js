...
M.atto_recordrtc.commonmodule = {
    ...
    handle_stop: function() {
        ...
        if (am.navigator_is_safari()) {
            cm.player.ancestor().addClass('hide');
            cm.replayer.ancestor().removeClass('hide');

            cm.replayer.setHTML('<source src="' + window.URL.createObjectURL(blob) + '" type="' + cm.recType + '/mp4">');
        }
    ...
    start_recording: function(type, stream) {
        ...
        if (am.navigator_is_safari()) {
            cm.replayer.ancestor().addClass('hide');
            cm.player.ancestor().removeClass('hide');
        }
    ...
    upload_to_server: function(type, callback) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) { // If src media was successfully retrieved.
                ...
                var filenamePostfix = '';
                if (type === 'audio') {
                    filenamePostfix = am.navigator_is_safari()
                        ? '-audio.m4a'
                        : '-audio.ogg';
                } else {
                    filenamePostfix = am.navigator_is_safari()
...
M.atto_recordrtc.compatcheckmodule = {
    ...
    check_has_gum: function() {
        if (!(navigator.mediaDevices && window.MediaRecorder)) {
            if (am.navigator_is_safari()) {
                am.show_alert('nowebrtcsafari', function() {
...
M.atto_recordrtc.abstractmodule = {
    ...
    // Check whether we're on Safari browser.
    navigator_is_safari: function() {
    ...
    select_rec_options: function(recType) {
        ...
        var compatTypes = types.filter(function(type) {
            if (typeof window.MediaRecorder.isTypeSupported == 'undefined') {
                return false;
            }
        ...
        else if (am.navigator_is_safari()) {
            options.mimeType = recType + '/mp4';
        }
...
M.atto_recordrtc.audiomodule = {
    init: function(scope) {
        ...
        cm.replayer = Y.one('audio#replayer');
...
M.atto_recordrtc.videomodule = {
    init: function(scope) {
        ...
        cm.replayer = Y.one('video#replayer');
